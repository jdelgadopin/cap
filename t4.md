...
Veiem que `(map inc (range 10)) ≡ (reduce (map-reducer inc) [] (range 10))`

Fixeu-vos que, donada una transformació `f` sobre cada element d'una seqüència, la funció
`map-reducer` retorna una funció-_reduce_:

```Clojure
(map-reduce f) ⇨ (fn [resultat valor] (conj resultat (f valor)))
```
...
de [Rich Hickey](https://www.linkedin.com/in/richhickey), principalment.]
